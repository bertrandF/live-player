...
usage(char* name)
{
    ...
    fprintf(stdout, "\n");
    fprintf(stdout, "  -d, --device <n>      Index of video device to use (defaults to %d)\n", DEFAULT_DEVICE);
...
main(int argc, char** argv )
{
    ...
    // Open input file
    if( avformat_open_input(&fmtCtx, infile, 0, NULL)!=0 ) {
    ...
    for(i=0 ; i<loops && !end ; ++i) {
        ...
        if( packet.stream_index==videoStreamIdx ) {
            ...
            if( finishedFrame ) {
                ...
                av_picture_copy(&pict, frame, frame->format, codecCtx->width,
                        codecCtx->height);
